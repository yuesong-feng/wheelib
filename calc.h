...
static inline bool uint16_add_overflow(uint16_t a, uint16_t b,
                                       uint16_t *result) {
  uint16_t res = a + b;
  overflow_if(res < a);
  *result = res;
...
static inline bool uint32_add_overflow(uint32_t a, uint32_t b,
                                       uint32_t *result) {
  uint32_t res = a + b;
  overflow_if(res < a);
  *result = res;
...
static inline bool uint64_add_overflow(uint64_t a, uint64_t b,
                                       uint64_t *result) {
  uint64_t res = a + b;
  overflow_if(res < a);
  *result = res;
...
static inline bool uint16_sub_overflow(uint16_t a, uint16_t b,
                                       uint16_t *result) {
  overflow_if(b > a);
  *result = a - b;
  return false;
...
static inline bool uint32_sub_overflow(uint32_t a, uint32_t b,
                                       uint32_t *result) {
  overflow_if(b > a);
  *result = a - b;
  return false;
...
static inline bool uint64_sub_overflow(uint64_t a, uint64_t b,
                                       uint64_t *result) {
  overflow_if(b > a);
  *result = a - b;
  return false;
...
static inline bool int64_mul_overflow(int64_t a, int64_t b, int64_t *result) {
  overflow_if(
      (a > INT32_MAX || a < INT32_MIN || b > INT32_MAX || b < INT32_MIN) &&
      a != 0 && a != 1 && b != 0 && b != 1 &&
      ((a > 0 && b > 0 && a > INT64_MAX / b) ||
...
static inline bool uint16_mul_overflow(uint16_t a, uint16_t b,
                                       uint16_t *result) {
...
static inline bool uint32_mul_overflow(uint32_t a, uint32_t b,
                                       uint32_t *result) {
...
static inline bool uint64_mul_overflow(uint64_t a, uint64_t b,
                                       uint64_t *result) {
  uint64_t res = a * b;
...
#define ULINT_UNDEFINED ((unsigned long int)(-1))
#define ULINT_MAX ((unsigned long int)(-2))
#define INFINITE_TIME ULINT_UNDEFINED
static inline void calc_abstime(
    unsigned long int
        time_in_usec /*timeout in microseconds, or INFINITE_TIME*/,
    struct timespec *abstime) {
  const unsigned long int MICROSECS_IN_A_SECOND = 1000000;
  if (time_in_usec != INFINITE_TIME) {
    ...
    if ((unsigned long int)tv.tv_usec >= MICROSECS_IN_A_SECOND) {
