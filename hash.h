...
{
  wl_ad(table);
  wl_ad(table->magic_n == HASH_TABLE_MAGIC_N);
...
static inline void hash_table_init(hash_table_t *table, size_t n) {
  uint64_t prime = find_prime(n);
  table->cells = malloc(prime * sizeof(hash_cell_t));
  table->n_cells = prime;
...
static inline void hash_table_destroy(hash_table_t *table) {
  wl_ad(table->magic_n == HASH_TABLE_MAGIC_N);
}
...
static inline uint64_t hash_calc_cell_id(uint64_t hash_value, hash_table_t const *table) {
  wl_ad(table);
  wl_ad(table->magic_n == HASH_TABLE_MAGIC_N);
...
#ifdef WLIB_DEBUG
  #define HASH_ASSERT_VALID(DATA) wl_a((void *)(DATA) != (void *)-1)
...
uint64_t tab_hash_lookup_table[256][8] = {
...
static inline uint64_t hash_uint64(uint64_t value) {
  ...
  for (size_t i = 0; i < sizeof(uint64_t); ++i) {
    res ^= tab_hash_lookup_table[(value >> (i * 8)) & 0xFF][i];
  }
  return res;
}
...
uint32_t hash_uint32_pair_ib(uint32_t n1, uint32_t n2) {
  uint32_t HASH_RANDOM_MASK = 1463735687;
  uint32_t HASH_RANDOM_MASK2 = 1653893711;
...
and x64. It may be different for SPARC (and in particular hash_read_from_8(&some_u64)
...
static inline int64_t hash_read_from_1(const byte *addr) { return *addr; }
static inline int64_t hash_read_from_2(const byte *addr) {
  return hash_read_from_1(addr) | (hash_read_from_1(addr + 1) << 8);
}
static inline int64_t hash_read_from_4(const byte *addr) {
  return hash_read_from_2(addr) | (hash_read_from_2(addr + 2) << 16);
}
static inline int64_t hash_read_from_8(const byte *addr) {
  return hash_read_from_4(addr) | (hash_read_from_4(addr + 4) << 32);
}
...
@return hashed value */
static inline uint64_t hash_binary(const byte *str, size_t len /*, uint64_t seed = 0xacb1f3526e25dd39*/) {
  ...
  //   uint64_t h = hash_uint64_pair(seed, hash_read_from_8(str));
  ...
  */
  ...
  size_t i = 0;
  if (len & 8) {
    h = hash_uint64_pair(h, hash_read_from_8(str + i));
    i += 8;
  }
  ...
  if (len & 4) {
    last_part |= hash_read_from_4(str + i) << (16 + 8);
    i += 4;
  }
  if (len & 2) {
    last_part |= hash_read_from_2(str + i) << 8;
    i += 2;
  }
  if (len & 1) {
    last_part |= hash_read_from_1(str + i);
    i += 1;
  }
  wl_ad(i == len);
...
static inline uint64_t hash_string(const char *str) {
  return hash_binary((const byte *)(str), strlen(str));
}
...
static inline uint32_t hash_binary_ib(const byte *str, size_t len) {
  ...
  while (str < str_end) {
    hash_value = hash_uint32_pair_ib(hash_value, *str++);
    hash_value = hash_uint32_pair_ib(hash_value, *str++);
    hash_value = hash_uint32_pair_ib(hash_value, *str++);
    hash_value = hash_uint32_pair_ib(hash_value, *str++);
    hash_value = hash_uint32_pair_ib(hash_value, *str++);
